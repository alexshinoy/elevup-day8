  ...
  return (
    <aside className={`sidebar ${isOpen ? 'open' : ''}`} style={{ width: '220px', padding: '15px', borderRight: `1px solid var(--border)`, backgroundColor: 'var(--bg-surface)' }}>
      <ul style={{ listStyleType: 'none', padding: 0 }}>
        ...
        <li style={{ marginBottom: '5px' }}><Link to="/subscriptions" style={getStyle('/subscriptions')}><PlaySquare size={22}/> Subscriptions</Link></li>
        <li style={{ marginBottom: '5px' }}><Link to="/history" style={getStyle('/history')}><Clock size={22}/> History</Link></li>
      </ul>
    </aside>
  );
}